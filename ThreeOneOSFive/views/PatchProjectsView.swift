...
struct PatchProjectsView: View {
    ...
    @State private var showDebugConsole = true
    ...
    private var hasLocalContent: Bool {
        !store.items.isEmpty || !wallpaperPackages.isEmpty || !remoteItems.isEmpty
    }

    private var hasSearchResults: Bool {
        !filteredItems.isEmpty || !filteredWallpaperPackages.isEmpty || !remoteItems.isEmpty
    }
                    ...
                    if showDebugConsole {
                        Section("🛠 NHẬT KÝ KẾT NỐI WEB & ĐỒNG BỘ") {
                            VStack(alignment: .leading, spacing: 6) {
                                HStack {
                                    Text("Hệ thống Log Web API").font(.system(size: 11, weight: .bold, design: .monospaced)).foregroundColor(.yellow)
                                    Spacer()
                                    Button("Xóa log") { debugLogs.removeAll() }.font(.caption2).foregroundColor(.gray)
                                }
                                ForEach(debugLogs.prefix(8), id: \.self) { log in
                        ...
                        if !remoteItems.isEmpty {
                            Section("🌐 TÍNH NĂNG TỪ WEB (SERVER)") {
                                ForEach(remoteItems) { item in
                                    WebRemoteAimRow(remoteItem: item, store: store, onLog: { msg in appendLog(msg) })
                                }
                            }
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        Button {
                            ...
                            Label("Lấy dữ liệu từ Web", systemImage: "network")
                        }
                        Button {
                            showCreate = true
                        } label: {
                        ...
                        if store.isBusy || isImportingWallpapers || isImportingPatches || isFetchingRemote {
                            ProgressView()
                        } else {
    ...
    @MainActor private func fetchRemoteData() async {
        ...
        do {
            appendLog("🌐 Đang kết nối lấy dữ liệu từ server...")
            ...
            appendLog("❌ [SYNC] Lỗi tải danh sách từ web: \(error.localizedDescription)")
        }
    }
...
struct WebRemoteAimRow: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ZStack {
                RoundedRectangle(cornerRadius: 12).fill(Color.accentColor.opacity(0.12)).frame(width: 42, height: 42)
                Image(systemName: isApplied ? "checkmark.shield.fill" : "globe").foregroundColor(.accentColor)
            }
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 6) {
                    Text(remoteItem.name).font(.system(size: 15, weight: .bold)).foregroundStyle(.primary)
                    Text("ID: \(remoteItem.id)").font(.system(size: 9, weight: .bold)).padding(.horizontal, 6).padding(.vertical, 2).background(Color.accentColor.opacity(0.2)).cornerRadius(4).foregroundColor(.accentColor)
                }
                if let note = remoteItem.note, !note.isEmpty {
                    Text("📌 \(note)").font(.system(size: 11, design: .monospaced)).foregroundStyle(.secondary)
                }
            }
            Spacer()
            if isWorking {
        ...
        Task.detached(priority: .userInitiated) {
            do {
                ...
                await MainActor.run {
                    ...
                    onLog("❌ [LỖI WEB API]: \(error.localizedDescription)")
                }
            }
        }
    }
...
// MARK: - 4. UTILITIES & SUPPORT STRUCTURES
