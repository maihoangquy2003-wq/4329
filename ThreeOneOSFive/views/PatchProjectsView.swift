        ...
        ZStack {
            ...
            VStack(spacing: 28) {
                ...
                VStack(spacing: 10) {
                    ...
                    Text("ĐANG KÍCH HOẠT\(dots)").font(.system(size: 16, weight: .heavy)).tracking(2.5)
                        .foregroundStyle(.white)
                    Text("Vui lòng không thoát ứng dụng").font(.system(size: 11, weight: .medium))
...
enum PatchMetaStore {
    ...
    static func remove(localName: String) {
        var d = all()
        if d.removeValue(forKey: localName) != nil { save(d) }
    }
}
    ...
    var body: some View {
        ZStack {
            Circle().fill(Color.white.opacity(0.15))
                .frame(width: pulse ? 128 : 110, height: pulse ? 128 : 110).blur(radius: 22)
            Circle().strokeBorder(Color.white.opacity(0.25), style: StrokeStyle(lineWidth: 1, dash: [3, 6]))
                .frame(width: 116, height: 116).rotationEffect(.degrees(rotate ? 360 : 0))
            Circle().strokeBorder(LinearGradient(colors: [.white, .white.opacity(0.3), .white],
...
private struct ChevronCircle: View {
    var body: some View {
        ZStack {
            Circle().fill(.white).frame(width: 38, height: 38).shadow(color: .white.opacity(0.5), radius: 10)
...
private struct EmptyStateView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ZStack {
                Circle().strokeBorder(.white.opacity(0.25), style: StrokeStyle(lineWidth: 1.4, dash: [3, 5]))
                    .frame(width: 84, height: 84).rotationEffect(.degrees(rot ? 360 : 0))
...
struct ActivationNoteSheet: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 22) {
                    ...
                    if isError {
                        VStack(spacing: 10) {
                            Text("HEADLOCK ZENIS").font(.system(size: 11, weight: .heavy)).tracking(4.5).foregroundStyle(.white.opacity(0.55))
                            ...
                            Text(info.patchName).font(.system(size: 15, weight: .heavy)).foregroundStyle(.white)
                                .multilineTextAlignment(.center).padding(.horizontal, 28)
                            if !info.tag.isEmpty { TagPill(tag: info.tag) }
                        }
                    } else {
                        VStack(spacing: 12) {
                            Text("HEADLOCK ZENIS").font(.system(size: 13, weight: .heavy)).tracking(4.5).foregroundStyle(.white.opacity(0.85))
                            ...
                            Text(info.patchName).font(.system(size: 18, weight: .heavy)).foregroundStyle(.white)
                    ...
                    if isError, let e = info.errorMessage, !e.isEmpty {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            Text(e).font(.system(size: 12.5, weight: .medium)).foregroundStyle(.white.opacity(0.9))
                    ...
                    if hasNote {
                        VStack(alignment: .leading, spacing: 14) {
                            ...
                            Button {
                                ...
                                .overlay(RoundedRectangle(cornerRadius: 12, style: .continuous).strokeBorder(.white, lineWidth: 1.4))
                            }.buttonStyle(.plain)
                        }
                        .padding(18)
                        .background(RoundedRectangle(cornerRadius: 16, style: .continuous).fill(Theme.surface))
                        .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous).strokeBorder(.white.opacity(0.8), lineWidth: 1.3))
                        .padding(.horizontal, 22)
                    }
                    Button { SoundFX.tap(); onDismiss() } label: {
                        Text("ĐÃ HIỂU").font(.system(size: 14, weight: .heavy)).tracking(3).foregroundStyle(.black)
        ...
        .onAppear { withAnimation(.spring(response: 0.5, dampingFraction: 0.7)) { animateIn = true } }
    }
}
...
struct PatchProjectsView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear { SilentAlertBlocker.install(); store.reload() }
            .task {
    ...
    private var header: some View {
        VStack(spacing: 0) {
            ...
            Text("ZENITH SOLITUDE").font(.system(size: 22, weight: .black, design: .serif))
    ...
    private var syncPill: some View {
        HStack(spacing: 7) {
            ...
            Text(isSyncing ? "ĐANG CẬP NHẬT" : "ĐÃ KẾT NỐI").font(.system(size: 9, weight: .heavy)).tracking(1.6)
...
final class SyncEngine {
    ...
    func run(store: PatchProjectStore) async {
        ...
        let remoteFiles = Set(remotes.map { $0.filename })

        // ─── 1) Cập nhật meta cho file đã có nhưng chưa có meta ───
        for item in items {
