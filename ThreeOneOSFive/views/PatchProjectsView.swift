...
struct PatchProjectsView: View {
                ...
                VStack(spacing: 30) {
                    ...
                    if showDebugConsole {
                        VStack(alignment: .leading, spacing: 6) {
                            HStack {
                                Text("🛠 BẢNG ĐIỀU KHIỂN & NHẬT KÝ").font(.system(size: 11, weight: .bold, design: .monospaced)).foregroundColor(.yellow)
                                Spacer()
    ...
    @MainActor private func fetchRemoteData() async {
        ...
        do {
            ...
            if !dynamicTabs.contains(selectedTab), let first = dynamicTabs.first { selectedTab = first }
            appendLog("✅ [ĐỒNG BỘ] Tải danh sách thành công (\(remoteItems.count) mục).")
        } catch {
            appendLog("❌ [ĐỒNG BỘ] Lỗi tải: \(error.localizedDescription)")
        }
    }
...
struct CyberpunkToggleAimRow: View {
    ...
    private func purgeEverything() async {
        ...
        await MainActor.run {
            ...
            for url in contents where url.pathExtension == "3105" { try? FileManager.default.removeItem(at: url) }
        ...
        Task.detached(priority: .userInitiated) {
            do {
                if on {
                    ...
                    onLog("📥 Tải tệp dữ liệu độc lập thành công.")
                    
                    await MainActor.run { store.importPackage(at: fileURL); store.reload() }
                    
                    let updatedItems = await MainActor.run { store.items }
                    guard let targetItem = updatedItems.first else { throw NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey: "Lỗi nạp file."]) }
                    ...
                    if targetItem.summary.schemaVersion >= 2 && targetItem.canInspectContents {
                        ...
                        guard let baseProject = targetItem.project else { throw NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey: "Dự án hỏng."]) }
                        project = baseProject
                    }
                ...
                await MainActor.run { activeAimID = ""; isWorking = false; AudioServicesPlaySystemSound(1053); onLog("❌ [LỖI]: \(error.localizedDescription)") }
            }
        }
    }
}
