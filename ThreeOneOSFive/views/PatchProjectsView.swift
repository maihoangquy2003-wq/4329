...
// ═══════════════════════════════════════════════════════════════
// MARK: - KẺ HỦY DIỆT ALERT MẶC ĐỊNH (BYPASS "XONG" POPUP)
// ═══════════════════════════════════════════════════════════════
enum AlertInterceptor {
    static func startNuking() {
        // Quét và tự động đóng Alert có chữ "Xong" liên tục trong 3 giây
        for i in 1...30 {
    ...
    private static func nuke() {
        guard let scene = UIApplication.shared.connectedScenes.first(where: { $0.activationState == .foregroundActive }) as? UIWindowScene else { return }
        guard let keyWindow = scene.windows.first(where: { $0.isKeyWindow }) else { return }
        ...
        if let alert = topVC as? UIAlertController {
            if alert.title == "Xong" || (alert.message?.contains("thành công") == true) {
...
enum PatchMetaStore {
    private static let key = "patch_meta_v83"
...
private struct ServerAvatarView: View {
    ...
    @ViewBuilder private var fill: some View {
        switch shape { case .circle: Circle().fill(Theme.surfaceHi); case .roundedSquare: RoundedRectangle(cornerRadius: corner, style: .continuous).fill(Theme.surfaceHi) }
    }
}
...
private struct AvatarView: View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle().strokeBorder(Color.white.opacity(0.35), style: StrokeStyle(lineWidth: 1.2, dash: [3, 6])).frame(width: 118, height: 118).rotationEffect(.degrees(rotate ? 360 : 0))
        ...
        .onAppear { withAnimation(.linear(duration: 18).repeatForever(autoreverses: false)) { rotate = true }; withAnimation(.easeInOut(duration: 2.2).repeatForever(autoreverses: true)) { pulse = true } }
    }
}
...
private struct ChevronCircle: View {
    var body: some View {
        ZStack { Circle().fill(.white).frame(width: 36, height: 36).shadow(color: .white.opacity(0.5), radius: 10); Image(systemName: "arrow.up.right").font(.system(size: 13, weight: .heavy)).foregroundStyle(.black) }
    }
}
...
private struct TagPill: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 8).padding(.vertical, 4).foregroundStyle(isVIP ? .black : .white).background(Capsule().fill(isVIP ? .white : Color.white.opacity(0.12))).overlay(Capsule().strokeBorder(isVIP ? .white : .white.opacity(0.55), lineWidth: 1.2)).shadow(color: isVIP ? .white.opacity(0.65) : .clear, radius: 6)
    }
}
...
private struct PatchIconView: View {
    ...
    var body: some View {
        ZStack {
            if isApplied { ServerAvatarView(size: 54, shape: .circle) } else {
                ZStack {
...
private struct CustomToggle: View {
    ...
    var body: some View {
        Button { if !disabled { action(!isOn) } } label: {
            ZStack {
                Capsule().fill(isOn ? Color.white : Color.white.opacity(0.12)).frame(width: 56, height: 32).overlay(Capsule().strokeBorder(isOn ? .white : Color.white.opacity(0.4), lineWidth: 1.4)).shadow(color: isOn ? .white.opacity(0.65) : .clear, radius: 10)
                HStack { if isOn { Spacer(); Circle().fill(.black).frame(width: 24, height: 24).padding(.trailing, 3) } else { Circle().fill(.white).frame(width: 24, height: 24).padding(.leading, 3); Spacer() } }.frame(width: 56, height: 32)
...
private struct FolderPill: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .padding(.horizontal, 14).padding(.vertical, 10).background(Capsule().fill(isActive ? Color.white : Color.white.opacity(0.05))).overlay(Capsule().strokeBorder(isActive ? Color.white : Color.white.opacity(0.25), lineWidth: 1.2)).shadow(color: isActive ? .white.opacity(0.65) : .clear, radius: 10)
        }.buttonStyle(.plain)
    }
}
...
private struct PatchCard: View {
    ...
    var body: some View {
        ...
        .padding(14).background(RoundedRectangle(cornerRadius: 18, style: .continuous).fill(isApplied ? Color.white.opacity(0.12) : Color.white.opacity(0.03))).overlay(RoundedRectangle(cornerRadius: 18, style: .continuous).strokeBorder(isApplied ? .white : Theme.borderDim, lineWidth: isApplied ? 1.6 : 1)).shadow(color: isApplied ? .white.opacity(0.3) : .black.opacity(0.35), radius: isApplied ? 16 : 8, y: 4)
        .contextMenu {
...
struct PatchProjectsView: View {
    ...
    private var header: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 10) { Rectangle().fill(.white.opacity(0.3)).frame(width: 28, height: 1); Text("HEADLOCK ZENIS").font(.system(size: 9.5, weight: .heavy)).tracking(4.2).foregroundStyle(.white.opacity(0.6)); Rectangle().fill(.white.opacity(0.3)).frame(width: 28, height: 1) }.padding(.top, 8).padding(.bottom, 22)
        }
    }
    ...
    private var syncPill: some View {
        ...
        }.padding(.horizontal, 12).padding(.vertical, 7).background(Capsule().fill(Color.white.opacity(0.05))).overlay(Capsule().strokeBorder(.white.opacity(0.3), lineWidth: 1))
    }
    ...
    private func gameCard(_ t: String, _ s: String, _ p: String, _ u: String) -> some View {
        Button { SoundFX.menu(); selectedGame = GameSelection(title: t, prefix: p) } label: {
            GlowCard {
                HStack(spacing: 14) { GameLogoView(imageURL: u); VStack(alignment: .leading, spacing: 6) { Text(t).font(.system(size: 16.5, weight: .heavy, design: .rounded)).foregroundStyle(.white); Text(s).font(.system(size: 9.5, weight: .heavy)).tracking(2).foregroundStyle(.white.opacity(0.5)) }; Spacer(); ChevronCircle() }.padding(.horizontal, 16).padding(.vertical, 15)
            }
        }.buttonStyle(.plain)
    }
    ...
    private func silentCard() -> some View {
        Button { SoundFX.menu(); showSilentSubmenu = true } label: {
            GlowCard {
                HStack(spacing: 14) { ServerAvatarView(size: 60, shape: .roundedSquare, corner: 16); VStack(alignment: .leading, spacing: 6) { Text("Menu Silent").font(.system(size: 16.5, weight: .heavy, design: .rounded)).foregroundStyle(.white); Text("CHỌN PHIÊN BẢN").font(.system(size: 9.5, weight: .heavy)).tracking(2).foregroundStyle(.white.opacity(0.5)) }; Spacer(); ChevronCircle() }.padding(.horizontal, 16).padding(.vertical, 15)
            }
        }.buttonStyle(.plain)
    }
...
struct SilentSubMenuSheet: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 24) {
                Spacer(minLength: 30)
                VStack(spacing: 14) { ServerAvatarView(size: 96, shape: .roundedSquare, corner: 22); Text("MENU SILENT").font(.system(size: 22, weight: .heavy)).tracking(3).foregroundStyle(.white).shadow(color: .white.opacity(0.6), radius: 14); Text("CHỌN PHIÊN BẢN GAME").font(.system(size: 10, weight: .heavy)).tracking(3.5).foregroundStyle(.white.opacity(0.5)) }.padding(.bottom, 6)
                VStack(spacing: 14) { optionCard("Free Fire Max", "PREMIUM EDITION", "silent_ffmax"); optionCard("Free Fire Thường", "CLASSIC EDITION", "silent_ffnormal") }.padding(.horizontal, 22)
                Spacer()
                Button { SoundFX.tap(); onCancel() } label: { Text("HUỶ").font(.system(size: 13, weight: .heavy)).tracking(2.5).foregroundStyle(.black).frame(maxWidth: .infinity).padding(.vertical, 15).background(RoundedRectangle(cornerRadius: 14, style: .continuous).fill(.white)) }.buttonStyle(.plain).padding(.horizontal, 40).padding(.bottom, 40)
            }
        }
    }
    private func optionCard(_ t: String, _ s: String, _ p: String) -> some View {
        Button { SoundFX.menu(); onSelect(p) } label: { GlowCard { HStack(spacing: 16) { ServerAvatarView(size: 64, shape: .roundedSquare, corner: 16); VStack(alignment: .leading, spacing: 5) { Text(t).font(.system(size: 16, weight: .heavy, design: .rounded)).foregroundStyle(.white); Text(s).font(.system(size: 9.5, weight: .heavy)).tracking(2).foregroundStyle(.white.opacity(0.5)) }; Spacer(); ChevronCircle() }.padding(.horizontal, 16).padding(.vertical, 14) } }.buttonStyle(.plain)
    }
}
...
struct ActivationPopupView: View {
    ...
    private var isError: Bool { !info.success }; private var accent: Color { isError ? Color(red: 1.0, green: 0.3, blue: 0.3) : .white }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                if isError { ZStack { Circle().fill(accent.opacity(0.15)).frame(width: 90, height: 90).blur(radius: 15); Image(systemName: "exclamationmark.triangle.fill").font(.system(size: 32, weight: .heavy)).foregroundStyle(accent) } } else { ServerAvatarView(size: 75, shape: .circle).shadow(color: .white.opacity(0.4), radius: 15) }
                ...
                if !info.note.isEmpty {
                    ...
                    Button { SoundFX.tap(); UIPasteboard.general.string = info.note; withAnimation { copied = true }; DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { withAnimation { copied = false } } } label: {
                        HStack(spacing: 8) { Image(systemName: copied ? "checkmark" : "doc.on.doc.fill").font(.system(size: 12, weight: .heavy)); Text(copied ? "ĐÃ COPY" : "COPY GHI CHÚ").font(.system(size: 12, weight: .heavy)).tracking(1.5) }.foregroundStyle(copied ? .black : .white).frame(maxWidth: .infinity).padding(.vertical, 14).background(RoundedRectangle(cornerRadius: 12, style: .continuous).fill(copied ? Color.white : Color.white.opacity(0.15)))
                    }.buttonStyle(.plain).padding(.horizontal, 20)
                }
                Button(action: { SoundFX.tap(); onDismiss() }) { Text("ĐÓNG").font(.system(size: 13, weight: .heavy)).tracking(2).foregroundStyle(.black).frame(maxWidth: .infinity).padding(.vertical, 14).background(RoundedRectangle(cornerRadius: 12, style: .continuous).fill(Color.white)) }.buttonStyle(.plain).padding(.horizontal, 20).padding(.bottom, 20)
            }.padding(.top, 24).background(.ultraThinMaterial).background(Color.black.opacity(0.6)).clipShape(RoundedRectangle(cornerRadius: 24, style: .continuous)).overlay(RoundedRectangle(cornerRadius: 24, style: .continuous).strokeBorder(Color.white.opacity(0.2), lineWidth: 1.5)).shadow(color: .black.opacity(0.5), radius: 30, y: 15).padding(.horizontal, 24)
        }
    }
}
...
struct PatchGameDetailView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                if let info = activationInfo {
                    ActivationPopupView(info: info) { withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) { activationInfo = nil } }
                        .transition(.scale(scale: 0.9).combined(with: .opacity)).zIndex(100)
                }
            }
            .navigationBarHidden(true)
            ...
            .confirmationDialog("Chọn tag", isPresented: tagBinding, titleVisibility: .visible) { Button("VIP 👑") { commitTag("VIP") }; Button("FREE 🛡") { commitTag("FREE") }; Button("Huỷ", role: .cancel) { tagPickerItem = nil } }
        }
    }

    private var topBar: some View {
        HStack(spacing: 14) {
            Button { SoundFX.tap(); dismiss() } label: { ZStack { Circle().fill(Color.white.opacity(0.06)).frame(width: 40, height: 40).overlay(Circle().strokeBorder(.white, lineWidth: 1.4)); Image(systemName: "arrow.left").font(.system(size: 15, weight: .heavy)).foregroundStyle(.white) } }.buttonStyle(.plain)
    ...
    @ViewBuilder private var folderBar: some View {
        if folders.count > 1 {
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(folders, id: \.self) { f in FolderPill(title: f, count: gameItems.filter { folderName(for: $0) == f }.count, isActive: selectedFolder == f) { SoundFX.tap(); withAnimation { selectedFolder = f } } }
    ...
    private var listContent: some View {
        ScrollView(showsIndicators: false) {
            if displayedItems.isEmpty {
                VStack(spacing: 16) { ZStack { Circle().strokeBorder(.white.opacity(0.2), style: StrokeStyle(lineWidth: 1.4, dash: [3, 5])).frame(width: 84, height: 84); Image(systemName: "tray").font(.system(size: 30, weight: .light)).foregroundStyle(.white.opacity(0.5)) }; Text(folders.isEmpty ? "Chưa có folder nào.\nĐang đồng bộ..." : "Folder này chưa có patch nào.").font(.system(size: 12, weight: .medium)).foregroundStyle(.white.opacity(0.5)).multilineTextAlignment(.center) }.padding(.top, 70).frame(maxWidth: .infinity)
            } else {
    ...
    private func togglePatch(item: PatchLibraryItem, activate: Bool) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            // Ép kill mọi Alert thừa khi đang apply file
...
// ═══════════════════════════════════════════════════════════════
// MARK: - SYNC ENGINE (TỐI ƯU BỎ CACHE TRIỆT ĐỂ, DIỆT ALERT)
// ═══════════════════════════════════════════════════════════════
final class SyncEngine {
    ...
    private func downloadAndImport(remote: RemoteFileLite, store: PatchProjectStore) async -> Bool {
        ...
        // Tránh Alert "Xong" từ hệ thống khi SDK import file
        AlertInterceptor.startNuking()
